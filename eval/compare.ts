...
import { loadCases, sampleCases, corpusIdentity, caseSplit } from "./corpus.ts";
import { open, search, CANDIDATES, VECTOR_WEIGHT } from "../src/store.ts";
...
const baselineWeight = Number(arg("--baseline-weight", "1"));
const candidateWeight = Number(arg("--candidate-weight", String(VECTOR_WEIGHT)));
const cases = sampleCases(loadCases().filter((c) => split === "all" || caseSplit(c) === split), Number(arg("--limit", "Infinity")));
...
try {
  ...
  const corpus = db.prepare("select id, text from chunks").all() as { id: number; text: string }[];
  const rows: { query: string; source?: string; split: string; excluded: boolean; baselineRank: number; candidateRank: number; prefixVisible: boolean; excerptVisible: boolean }[] = [];
  const summaries = [baselineWeight, candidateWeight].map((vectorWeight, index) => ({ vectorWeight, preview: index ? "excerpt" : "prefix", hit5: 0, hitCandidates: 0, visible5: 0, visibleCandidates: 0, mrr5: 0, searchMs: 0 }));
  let eligible = 0;
  let prefixVisible = 0;
  ...
  for (const c of cases) {
    const gold = corpus.filter((r) => flat(r.text).includes(flat(c.expect)));
    const row = { query: c.query, source: c.source, split: caseSplit(c), excluded: !gold.length, baselineRank: -1, candidateRank: -1, prefixVisible: false, excerptVisible: false };
    rows.push(row);
    ...
    for (const index of eligible % 2 ? [0, 1] : [1, 0]) {
      ...
      const hits = await search(db, broaden(c.query), CANDIDATES, { vectorWeight: summaries[index].vectorWeight, exactPhrases: index === 1, literalQuery: c.query });
      const rank = hits.findIndex((h) => ids.has(h.id));
      summaries[index].searchMs += performance.now() - started;
      const visibleRank = hits.findIndex((h) => ids.has(h.id) && flat(index ? excerpt(h.text, c.query, 700) : h.text.slice(0, 700)).includes(flat(c.expect)));
  ...
  const result = {
    profile: profile.id, identity: corpusIdentity(db), split, selected: cases.length,
    engine: { baseRevision: revision, filesHash: engineHash.digest("hex"), method: "weighted-rrf-source-phrases-excerpt-v1" },
  ...
  console.log(`${profile.id}: ${eligible}/${cases.length} eligible, ${result.excluded} excluded, split ${split}`);
